...
(async () => {
  ...
  const key = await Bun.file(path.join(__dirname, "crp-ky")).text();
  if (!key) return console.error("Ky file not found");

  if (argEncrypt) {
    const source = await Bun.file(path.join(__dirname, "crp-so")).text();
    ...
    await Bun.write(path.join(__dirname, "crp-en"), encrypted);
    console.info("Encrypted !");
    return;
  }

  if (argDecrypt) {
    const fileEncrypted = await Bun.file(path.join(__dirname, "crp-en")).text();
    ...
    await Bun.write(path.join(__dirname, "crp-de"), decrypted);
