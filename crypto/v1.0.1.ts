...
(async () => {
  ...
  const key = await fs.readFile(path.join(__dirname, "crp-ky"), "utf-8");
  if (!key) return console.error("Ky file not found");

  if (argEncrypt) {
    const source = await fs.readFile(path.join(__dirname, "crp-so"), "utf-8");
  ...
  if (argDecrypt) {
    const fileEncrypted = await fs.readFile(path.join(__dirname, "crp-en"), "utf-8");
    ...
    await fs.writeFile(path.join(__dirname, "crp-dec"), decrypted);
